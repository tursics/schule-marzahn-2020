...
(function () {
	...
	ddj.marker = {
		...
		update: function () {
			var key, val, obj, addObj, uniqueSetId = [], uniqueSetKey = [], uniqueId = ddj.marker.settings.uniqueIdentifier;
			...
			for (key = 0; key < ddj.getData().length; ++key) {
				...
				if ((uniqueId !== undefined) && (uniqueId !== '') && (uniqueSetId.indexOf(val[uniqueId]) > -1)) {
					if (ddj.marker.settings.onDuplicate) {
						ddj.marker.settings.onDuplicate(val[uniqueId], ddj.getData(uniqueSetKey[uniqueSetId.indexOf(val[uniqueId])]));
					}
					continue;
				}
