...
(function () {
	...
	ddj.marker = {
		...
		update: function () {
			var key, val, obj, addObj, uniqueSet = [], uniqueId = ddj.marker.settings.uniqueIdentifier;
			...
			for (key = 0; key < ddj.getData().length; ++key) {
				...
				if ((uniqueId !== undefined) && (uniqueId !== '') && (uniqueSet.indexOf(val[uniqueId]) > -1)) {
					continue;
				}
